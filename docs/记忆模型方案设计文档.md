...
| 手段 | 作用 |
|------|------|
| `memory_utils.load_tokenizer` / `load_tokenizer_for_inference` | 训练与推理入口；推理可走 **本地目录** 优先。 |
| 环境变量 **`MEMORY_TOKENIZER_PATH`** | 指向已下载完整的 tokenizer 目录，绕过在线拉取。 |
| 目录 **`记忆模型/1.0/hf_tokenizer/`** | 各脚本内置默认本地目录名；将 `tokenizer.json`、`tokenizer_config.json`、`special_tokens_map.json`、**`tokenizer.model` 或 sentencepiece 相关**、`chat_template.jinja` 等与**当前基座仓库**一致的文件放入。 |
| **`download_hf_tokenizer.py`** | 辅助从 Hub 拉取 tokenizer 到本地，适合无代理或镜像不稳定场景。 |
...
| **记忆库** | 扫描 **`session/`** 下所有 **`*.raw.txt`**，点击用 **`GET /memory-bank/raw-content?rel=<相对 session/ 的路径>`** 预览。 | `GET /memory-bank/raw-files`、`GET /memory-bank/raw-content` |

**落盘约定（与实现对齐）**

- **`session_staging/`**：每轮终答后追加文本块（`memory_pipeline_core.append_session_staging_turn`）；HTTP 会话对应文件名为 **`cli_<YYYYMMDD>_<HHMMSS>_<session_id>.txt`**，与 **`POST /sessions` 返回的 `session_id`**（32 位 hex）一致，便于目录检索。  
...
- **`session/`**：离线提纯产物树（如 `YYYY/MM/DD/*.raw.txt` 等），由 **`session_to_memory`** 写入；**记忆库**侧车只读展示 `*.raw.txt`。

其它：`GET /ping`、`GET /health` 自检；**`SessionState`** 含 `session_id`、`history`、`staging_path`、`chat_dump_path`、`created_at`。

**离线（写入与巩固）**

1. `memory_extract_cli` 或 staging 文本 → `session_to_memory` → `session/.../*.memory.json`、`*.raw.txt`、`*.qa.jsonl`。  
...
| 现象 | 可能原因 | 处理 |
|------|----------|------|
| tokenizer 中文 0 token | 镜像损坏、错用其它基座词表 | 本地 `hf_tokenizer` + `MEMORY_TOKENIZER_PATH`；删坏缓存 |
...
| `session_to_memory.py` | 离线提纯：会话 → memory JSON、raw、qa；**`BUILTIN_STAGING_ON_RUN`** / **`BUILTIN_NEGATIVE_QA`**；多文件 staging 时可选 **tqdm** 进度条。 |
| `train_memory.py` | 全量 QLoRA。 |
| `train_memory_replay.py` | 按日回放式巩固训练。 |
| `download_hf_tokenizer.py` | 拉 tokenizer 到本地。 |
