...
核心思路：对话经历 → 提纯为结构化记忆（raw + QA）→ 训练进模型权重（LoRA）→ 回忆时模型直接生成，而非外部数据库检索。本仓库为 **v0.2（2.0 架构）**，早期 1.0 版本见 git tag `v0.1`。
...
跨目录引用已内置 sys.path 引导（`core`/`serve`/`train`），IDE 右键运行任意入口脚本即可。

## 版本历史

| Tag | 内容 |
|---|---|
| `v0.1` | 1.0：LoRA 训练（train_memory/replay）、记忆提取、API 服务、用户画像 |
| `v0.2` | 2.0：会话 ID 管理、原始语料合并/去重、记忆提取与对话管线重构 |
