...
BUILTIN_SESSION_ROOT = str(_script_dir / "session_raw")
...
def build_incremental_messages(
    ...
    *,
    ...
    「当日」= 字典序最新的 ``YYYY-MM-DD`` 目录（全量 raw + QA）；历史池 = 其余日期目录，
    按 ``history_ratio`` 抽样追加（与当日样本不抢预算）。
    ``user_id`` 保留兼容——2.0 无用户目录，仅影响 main 的 output_dir 命名。"""
    date_dirs = tm._iter_session_date_dirs(session_root)
    if not date_dirs:
        raise ValueError(
            f"未找到任何 session_raw/YYYY-MM-DD 目录: {session_root}"
        )
    today_dir = date_dirs[-1]  # 字典序最新 = 当日
    latest_set = {today_dir.resolve()}
    historical_dirs = [d for d in date_dirs if d.resolve() not in latest_set]

    today_all: list[list[dict]] = []
    n_raw_today = 0
    n_qa_today = 0
    raws_t, qas_t = _collect_split_samples(today_dir)
    today_all.extend(raws_t)
    today_all.extend(qas_t)
    n_raw_today += len(raws_t)
    n_qa_today += len(qas_t)
    if not today_all:
        raise ValueError(f"当日日期目录下无训练样本: {today_dir}")
    ...
    _log(
        f"[train_memory_replay] 当日目录 {today_dir.name}：raw={n_raw_today} 条，QA={n_qa_today} 条；"
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="session 根目录（其下为 YYYY-MM-DD/，2.0 提纯输出 session_raw/）",
    )
    p.add_argument(
        "--user_id",
        type=str,
        default=BUILTIN_USER_ID,
        help="指定时仅影响 output_dir 命名（outputs/<user_id>/<LoRA 子目录>）；2.0 数据无用户层级，回放按 session_raw/ 日期目录进行。右键运行请改本文件 BUILTIN_USER_ID。",
    )
    p.add_argument(
        "--output_dir",
        type=str,
