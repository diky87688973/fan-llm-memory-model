...
BUILTIN_SESSION_DIR = str(_script_dir / "session_raw")
...
def apply_hf_runtime_env(args: argparse.Namespace) -> None:
    ...
    import train_memory_utils as _mem
...
_log("[train_memory] 正在加载 memory_utils（推理公共）与 train_memory_utils（训练专用） …")
...
from train_memory_utils import (
...
import train_memory_utils as memory_utils
...
def _iter_session_date_dirs(session_root: Path) -> list[Path]:
    """``session_root`` 为 ``session_raw/``（2.0 提纯输出）：返回全部 ``YYYY-MM-DD`` 日期目录（字典序）。"""
    if not session_root.is_dir():
        return []
    subs = [d for d in session_root.iterdir() if d.is_dir() and is_iso_date_dir_name(d.name)]
...
def collect_training_messages(
    ...
    """``session_root`` 为 ``session_raw/``（2.0 提纯输出，无用户层级）：
    汇总全部 ``YYYY-MM-DD`` 日期目录下的样本（raw + QA）。
    ``train_user_id`` 保留兼容——2.0 无用户目录，仅影响 main 的 output_dir 命名。"""
    rows: list[list[dict]] = []
    for day_dir in _iter_session_date_dirs(session_root):
        rows.extend(_collect_training_messages_single_dir(day_dir))
    if not rows:
        raise ValueError(
            "没有生成任何训练样本；请在 session_raw/YYYY-MM-DD/ 下放置 *.qa.jsonl、*.raw.txt"
            "（由 session_to_memory 提纯输出）。"
        )
    return rows
...
def collect_qa_pairs_for_probe(
    ...
    """从 ``session_raw/`` 全部日期目录下的 *.qa.jsonl 收集 (q,a)，供探针使用。"""
    pairs: list[tuple[str, str]] = []
    for day_dir in _iter_session_date_dirs(session_root):
        pairs.extend(_collect_qa_pairs_probe_one_dir(day_dir))
    return pairs
